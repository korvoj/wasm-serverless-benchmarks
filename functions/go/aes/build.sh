...
build_debian() {
  ...
  buildah push \
    --authfile $AUTHFILE \
    debian-go-aes \
    docker://$IMAGE_REPOSITORY/debian/go-aes:v1.0.0
}
...
build_distroless() {
  ...
  buildah push \
    --authfile $AUTHFILE \
    distroless-go-aes \
    docker://$IMAGE_REPOSITORY/distroless/go-aes:v1.0.0
}
...
build_wasmedge_jit() {
  buildah build \
    ...
    -t wasmedge-go-jit-aes .

  buildah push \
    --authfile $AUTHFILE \
    wasmedge-go-jit-aes \
    docker://$IMAGE_REPOSITORY/wasmedge/go-jit-aes:v1.0.0
}
...
build_wasmedge_aot() {
  buildah build \
    ...
    --authfile $AUTHFILE \
    --annotation "module.wasm.image/variant=compat-smart" \
    -t wasmedge-go-aot-aes .

  buildah push \
    --authfile $AUTHFILE \
    wasmedge-go-aot-aes \
    docker://$IMAGE_REPOSITORY/wasmedge/go-aot-aes:v1.0.0
}
...
build_wasmtime_jit() {
  buildah build \
    ...
    --authfile $AUTHFILE \
    --annotation "module.wasm.image/variant=compat-smart" \
    -t wasmtime-go-jit-aes .

  buildah push \
    --authfile $AUTHFILE \
    wasmtime-go-jit-aes \
    docker://$IMAGE_REPOSITORY/wasmtime/go-jit-aes:v1.0.0
}
...
build_wasmtime_aot() {
  buildah build \
    ...
    --authfile $AUTHFILE \
    --annotation "module.wasm.image/variant=compat-smart" \
    -t wasmtime-go-aot-aes .

  buildah push \
    --authfile $AUTHFILE \
    wasmtime-go-aot-aes \
    docker://$IMAGE_REPOSITORY/wasmtime/go-aot-aes:v1.0.0
}
...
build_wasmer_jit() {
  ...
  buildah push \
    --authfile $AUTHFILE \
    wasmer-go-jit-aes \
    docker://$IMAGE_REPOSITORY/wasmer/go-jit-aes:v1.0.0
}
...
build_wasmer_aot() {
  ...
  buildah push \
    --authfile $AUTHFILE \
    wasmer-go-aot-aes \
    docker://$IMAGE_REPOSITORY/wasmer/go-aot-aes:v1.0.0
}
