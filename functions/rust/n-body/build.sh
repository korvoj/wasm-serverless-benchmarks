...
build_debian() {
  buildah build \
    -f Dockerfile.debian \
    --authfile $AUTHFILE \
    -t debian-rust-n-body .

  buildah push \
    --authfile $AUTHFILE \
    debian-rust-n-body \
    docker://$IMAGE_REPOSITORY/debian/rust-n-body:v1.0.0
}
...
build_distroless() {
  buildah build \
    ...
    --authfile $AUTHFILE \
    -t distroless-rust-n-body .

  buildah push \
    --authfile $AUTHFILE \
    distroless-rust-n-body \
    docker://$IMAGE_REPOSITORY/distroless/rust-n-body:v1.0.0
}
...
build_wasmedge_jit() {
  buildah build \
    ...
    -t wasmedge-rust-jit-n-body .

  buildah push \
    --authfile $AUTHFILE \
    wasmedge-rust-jit-n-body \
    docker://$IMAGE_REPOSITORY/wasmedge/rust-jit-n-body:v1.0.0
}
...
build_wasmedge_aot() {
  buildah build \
    ...
    --authfile $AUTHFILE \
    --annotation "module.wasm.image/variant=compat-smart" \
    -t wasmedge-rust-aot-n-body .

  buildah push \
    --authfile $AUTHFILE \
    wasmedge-rust-aot-n-body \
    docker://$IMAGE_REPOSITORY/wasmedge/rust-aot-n-body:v1.0.0
}
...
build_wasmtime_jit() {
  buildah build \
    ...
    --authfile $AUTHFILE \
    --annotation "module.wasm.image/variant=compat-smart" \
    -t wasmtime-rust-jit-n-body .

  buildah push \
    --authfile $AUTHFILE \
    wasmtime-rust-jit-n-body \
    docker://$IMAGE_REPOSITORY/wasmtime/rust-jit-n-body:v1.0.0
}
...
build_wasmtime_aot() {
  buildah build \
    ...
    --authfile $AUTHFILE \
    --annotation "module.wasm.image/variant=compat-smart" \
    -t wasmtime-rust-aot-n-body .

  buildah push \
    --authfile $AUTHFILE \
    wasmtime-rust-aot-n-body \
    docker://$IMAGE_REPOSITORY/wasmtime/rust-aot-n-body:v1.0.0
}
...
build_wasmer_jit() {
  buildah build \
    ...
    --authfile $AUTHFILE \
    --annotation "module.wasm.image/variant=compat-smart" \
    -t wasmer-rust-jit-n-body .

  buildah push \
    --authfile $AUTHFILE \
    wasmer-rust-jit-n-body \
    docker://$IMAGE_REPOSITORY/wasmer/rust-jit-n-body:v1.0.0
}
...
build_wasmer_aot() {
  buildah build \
    ...
    --authfile $AUTHFILE \
    --annotation "module.wasm.image/variant=compat-smart" \
    -t wasmer-rust-aot-n-body .

  buildah push \
    --authfile $AUTHFILE \
    wasmer-rust-aot-n-body \
    docker://$IMAGE_REPOSITORY/wasmer/rust-aot-n-body:v1.0.0
}
