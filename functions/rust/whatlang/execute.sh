...
case $MODE in
  debian)
    ...
    IMAGE_NAME=debian/rust-whatlang
    WASM_BINARY_PATH=whatlang
    RUN_NAME=debian-rust-whatlang
    ;;
  distroless)
    echo -n "Executing as a distroless image..."
    IMAGE_NAME=distroless/rust-whatlang
    WASM_BINARY_PATH=whatlang
    RUN_NAME=distroless-rust-whatlang
    ;;
  wasmedge-jit)
    echo -n "Executing with WasmEdge JIT..."
    IMAGE_NAME=wasmedge/rust-jit-whatlang
    WASM_BINARY_PATH=whatlang.wasm
    RUN_NAME=wasmedge-rust-jit-whatlang
    ;;
  wasmedge-aot)
    echo -n "Executing with WasmEdge..."
    IMAGE_NAME=wasmedge/rust-aot-whatlang
    WASM_BINARY_PATH=whatlang.aot.wasm
    RUN_NAME=wasmedge-rust-aot-whatlang
    ;;
  wasmtime-jit)
    echo "Executing with WasmTime JIT..."
    IMAGE_NAME=wasmtime/rust-jit-whatlang
    WASM_BINARY_PATH=whatlang.wasm
    RUN_NAME=wasmtime-rust-jit-whatlang
    ;;
  wasmtime-aot)
    echo "Executing with WasmTime AOT..."
    IMAGE_NAME=wasmtime/rust-aot-whatlang
    WASM_BINARY_PATH=whatlang.aot.wasm
    RUN_NAME=wasmtime-rust-aot-whatlang
    ;;
  wasmer-jit)
    echo "Executing with Wasmer JIT..."
    IMAGE_NAME=wasmer/rust-jit-whatlang
    WASM_BINARY_PATH=whatlang.wasm
    RUN_NAME=wasmer-rust-jit-whatlang
    ;;
  wasmer-aot)
    echo "Executing with Wasmer AOT..."
    IMAGE_NAME=wasmer/rust-aot-whatlang
    WASM_BINARY_PATH=whatlang.aot.wasm
    RUN_NAME=wasmer-rust-aot-whatlang
    ;;
  *)
...
ctr run --rm --runc-binary crun \
  ...
  /$WASM_BINARY_PATH "Hi there, this is a message."
