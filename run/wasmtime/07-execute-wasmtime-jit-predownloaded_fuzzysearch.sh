...
REGISTRY_PASSWORD=$2
TEST_NAME=fuzzysearch
IMAGE_NAME=wasmtime/rust-aot-$TEST_NAME
IMAGE_TAG=v1.0.0
RUN_NAME=wasmtime-rust-aot-$TEST_NAME
...
do
  ...
  { /usr/bin/time -f '%e' ctr run --rm --runc-binary crun \
    ...
    /$WASM_BINARY_PATH /context/hamlet.txt Hamlet; } 2>> ../../results/wasmtime/aot/04-fuzzysearch.txt
  sleep 5
done
