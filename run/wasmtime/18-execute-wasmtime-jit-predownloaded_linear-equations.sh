...
IMAGE_NAME=wasmtime/go-jit-$TEST_NAME
IMAGE_TAG=v1.0.0
RUN_NAME=wasmtime-go-jit-$TEST_NAME
WASM_BINARY_PATH=linear-equations.wasm
...
do
  ...
  { /usr/bin/time -f '%e' ctr run --rm --runc-binary crun \
    ...
    /$WASM_BINARY_PATH 500; } 2>> ../../results/wasmtime/jit/09-linear-equations.txt
  sleep 5
done
