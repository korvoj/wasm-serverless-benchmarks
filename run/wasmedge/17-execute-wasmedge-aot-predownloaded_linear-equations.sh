...
REGISTRY_PASSWORD=$2
TEST_NAME=linear-equations
IMAGE_NAME=wasmedge/go-aot-$TEST_NAME
IMAGE_TAG=v1.0.0
RUN_NAME=wasmedge-go-aot-$TEST_NAME
WASM_BINARY_PATH=linear-equations.aot.wasm
...
do
  ...
  { /usr/bin/time -f '%e' ctr run --rm --runc-binary crun \
    ...
    /$WASM_BINARY_PATH 500; } 2>> ../../results/wasmedge/aot/09-linear-equations.txt
  sleep 5
done
